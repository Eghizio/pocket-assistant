...
const getUserReceipts = (userId: string, page = 1, limit = 7) => {
  const cappedLimit = Math.min(MAX_RECEIPTS_PAGE_LIMIT, limit);
  return TransactionRepository.getUserReceipts(userId, page, cappedLimit).map(
    TransactionMapper.toReceiptDto
  );
};
...
const addReceipt = (userId: string, date: Date, products: ProductPayload[]) => {
  ...
  const receipt = TransactionRepository.addReceiptWithProducts(
    userId,
    date,
    products
  );
...
const getUserProductsByReceipt = (receiptId: string) => {
  return TransactionRepository.getProductsByReceipt(receiptId).map(
    TransactionMapper.toProductDto
  );
};
