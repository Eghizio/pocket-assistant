...
const getUserBudgets = (userId: string, page: number = 1, limit: number = 12) => {
  const cappedLimit = Math.min(MAX_BUDGET_PAGE_LIMIT, limit);
  return BudgetRepository.getUserBudgets(userId, page, cappedLimit);
};

const getUserBudgetsWithinDate = (userId: string, after: Date, before: Date) => {
  return BudgetRepository.getUserBudgetsWithinDateRange(userId, after, before)
