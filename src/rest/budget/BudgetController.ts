...
import {
  BudgetBodySchema,
  BudgetDateRangeQuerySchema,
  BudgetPaginationQuerySchema,
} from "./validation";

const userId = "1"; // Get user ID from some session or token

const getAllBudgets = createValidatedHandler({
  query: BudgetPaginationQuerySchema,
})((req, res) => {
...
const getUserBudgetsWithinDate = createValidatedHandler({
  query: BudgetDateRangeQuerySchema,
})((req, res) => {
  const { after, before } = req.query;
  const budgetsWithinDate = BudgetService.getUserBudgetsWithinDate(
    userId,
    after,
    before
  );
...
// todo parse/validate
const addBudget: RequestHandler = createValidatedHandler({
